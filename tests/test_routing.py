...
import pytest
from ice_sul.routing import Coordinate, OSRMClient, RoutingError
...
def route_transport(url, timeout):
    ...
    return {"code":"Ok", "routes":[{"duration":1200,"distance":18000}], "waypoints":[{"location":[-49.269,-25.431]},{"location":[-49.171,-25.529]}]}
...
def test_table_batch_units_nulls_and_indices():
    def transport(url, timeout):
        ...
        return {"code":"Ok", "durations":[[60,None],[120,180]], "distances":[[1000,None],[2000,3000]],
                "sources":[{"location":[-49,-25]},{"location":[-50,-26]}],
                "destinations":[{"location":[-51,-27]},{"location":[-52,-28]}]}
