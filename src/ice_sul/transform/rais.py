...
def is_public_administration(cnae: object, legal_nature: object) -> bool:
    ...
    if not nature:
        raise ValueError(f"Natureza Jurídica vazia: {legal_nature!r}")
    # A tabela oficial usa códigos de quatro dígitos (três mais DV). Aceitar um
    # grupo desconhecido transformaria silenciosamente dado inválido em privado.
    if len(nature) != 4 or nature[0] not in {"1", "2", "3", "4", "5"}:
        raise ValueError(f"Natureza Jurídica desconhecida: {legal_nature!r}")
...
def transform_rows(
    ...
    for row in rows:
        qa["vinculos_lidos"] += 1
        active = _digits(row[columns["ativo"]])
        if active not in {"0", "1"}:
            raise ValueError(f"indicador de vínculo ativo desconhecido: {row[columns['ativo']]!r}")
        if active == "0":
