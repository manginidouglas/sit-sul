import math
import pytest
from ice_sul.mvp.scoring import (
    DegenerateIndicator,
    axis_score,
    minmax,
    overall,
...
def test_winsorization_preserves_na_and_flags_extremes():
    values, lo, hi, flags = winsorize([None, 0, 1, 2, 100], 0.25, 0.75)
    assert values == [None, 0.75, 1, 2, 26.5]
...
def test_minmax_directions_domain_and_degenerate():
    ...
    with pytest.raises(DegenerateIndicator):
        minmax([1, 1], "positiva")
...
def test_coverage_boundary_weight_renormalization_and_overall():
    ...
    assert coverage == 0.8 and score == 62.5
    assert overall(score, 50) == 56.25 and overall(None, 50) is None


def test_keys_duplicates_and_count():
    valid = [
        {"municipio_id": "1234567", "municipio_nome": "A", "uf_sigla": "PR"},
        {"municipio_id": "7654321", "municipio_nome": "B", "uf_sigla": "RS"},
    ]
    validate_municipal_keys(valid, valid, expected_count=2)
    with pytest.raises(ValueError):
        validate_municipal_keys(valid + [valid[0]], valid, expected_count=2)
