    ...
    for target, aliases in ALIASES.items():
        for alias in aliases:
            if _normal(alias) in normalized:
                output[target] = normalized[_normal(alias)]
                break
    missing = {"municipio", "cnae", "natureza", "ativo"} - output.keys()
    if missing:
...
def parse_active(value: object) -> str:
    return _strict(value, r"[01]", "indicador de vínculo ativo")


def parse_municipality(value: object) -> str:
    return _strict(value, r"\d{6}", "código municipal")


def parse_cnae(value: object) -> str:
    parsed = _strict(value, r"\d{5}", "CNAE 2.0 classe")
    if parsed[:2] == "00":
        raise ValueError(f"CNAE 2.0 classe inválida: {parsed!r}")
    return parsed


def parse_legal_nature(value: object) -> str:
    parsed = _strict(value, r"\d{4}", "Natureza Jurídica")
    if parsed[0] not in "12345":
        raise ValueError(f"Natureza Jurídica grupo desconhecido: {parsed!r}")
    return parsed


def cnae_division(value: object) -> str:
    return parse_cnae(value)[:2]


def is_public_administration(cnae: object, legal_nature: object) -> bool:
    return parse_cnae(cnae)[:2] == "84" or parse_legal_nature(
        legal_nature
    ).startswith("1")
...
def load_municipality_map(reference_csv: Path) -> dict[str, str]:
    ...
    for row in rows:
        ibge = _strict(row.get("municipio_id"), r"\d{7}", "municipio_id canônico")
        rais = ibge[:6]
        if rais in mapping and mapping[rais] != ibge:
            raise ValueError(f"chave RAIS ambígua: {rais}")
        mapping[rais] = ibge
        mapping[ibge] = ibge
    return mapping
...
def read_comt(path: Path) -> tuple[csv.DictReader, TextIO, dict[str, str]]:
    ...
    try:
        ...
        stream.close()
        raise ValueError("delimitador .comt inválido") from exc
    stream.seek(0)
    reader = csv.DictReader(stream, dialect=dialect)
    if not reader.fieldnames:
        stream.close()
        raise ValueError(".comt sem cabeçalho")
    return reader, stream, {"encoding": encoding, "delimitador": dialect.delimiter}
...
def transform_rows(
    ...
    *,
    ...
    year: int,
    allowed_ufs: Iterable[str],
    municipality_map: Mapping[str, str],
    south_municipalities: Iterable[str] = (),
    source_file: str = "fixture.7z",
) -> RaisResult:
    allowed = tuple(sorted(set(uf.upper() for uf in allowed_ufs)))
    valid_ufs = set(UF_PREFIX.values())
    if not allowed or not set(allowed) <= valid_ufs:
        raise ValueError(f"cobertura territorial inválida para a partição: {allowed}")
    ...
    active_by_uf: Counter[str] = Counter()

    for row in rows:
        qa["vinculos_lidos"] += 1
        try:
            active = parse_active(row[columns["ativo"]])
        except ValueError:
            qa["valores_invalidos"] += 1
            qa["falhas_campo_ativo"] += 1
            raise
        active_frequency[active] += 1
        if active == "0":
            qa["vinculos_inativos_excluidos"] += 1
            continue
        qa["vinculos_ativos_lidos"] += 1
        try:
            raw = parse_municipality(row[columns["municipio"]])
            cnae = parse_cnae(row[columns["cnae"]])
            nature = parse_legal_nature(row[columns["natureza"]])
        except ValueError:
            qa["valores_invalidos"] += 1
            raise
        ...
        if code is None:
            ...
            continue
        row_uf = UF_PREFIX.get(code[:2])
        if row_uf not in allowed:
            raise ValueError(
                f"UF do município fora da cobertura da partição: "
                f"{raw}->{code}, UF={row_uf}, permitidas={allowed}"
            )
        if "uf" in columns and str(row[columns["uf"]]).strip().upper() != row_uf:
            raise ValueError(
                f"UF da coluna diverge da UF derivada do município: {raw}"
            )
        active_by_uf[row_uf] += 1
        linked.add(code)
        division = cnae[:2]
        if division == "84" or nature.startswith("1"):
    ...
    for code in south:
        code_uf = UF_PREFIX.get(code[:2])
        if code_uf not in allowed:
            raise ValueError(
                f"município Sul encaminhado à partição errada: {code}, "
                f"UF={code_uf}, permitidas={allowed}"
            )

    provenance = {
        "fonte_id": "MTE-PDET-RAIS",
        "fonte_arquivo": source_file,
        "versao_fonte": f"RAIS {year} VINC_PUB",
    }
    private = [
        {
            "municipio_id": code,
            "empregos_formais_privados": total,
            "periodo_referencia": str(year),
    ...
    for code in sorted(south):
        ...
        value = (
            None
            if total == 0
            else 1 - sum((count / total) ** 2 for count in sectors[code].values())
        )
        diagnostic.append(
            {
                "municipio_id": code,
                "indicador_id": "MER-DIAG-01",
                "valor_bruto": value,
                "periodo_referencia": str(year),
                "flag_qualidade": (
                    "ausente"
    ...
    quality: dict[str, object] = {
        "ano": year,
        **dict(qa),
        "frequencias_vinculo_ativo": dict(sorted(active_frequency.items())),
        "vinculos_ativos_por_uf": dict(sorted(active_by_uf.items())),
        "ufs_permitidas": list(allowed),
        "valores_invalidos": qa["valores_invalidos"],
        "municipios_ligados": len(linked),
        "codigos_nao_ligados": dict(sorted(unmatched.items())),
        "municipios_nao_ligados": len(unmatched),
        "soma_municipal": sum(totals.values()),
    }
    quality["reconciliacao_ok"] = (
        quality["soma_municipal"] == qa["vinculos_privados_elegiveis"]
    )
    quality["reconciliacao_territorial_ok"] = qa[
        "vinculos_ativos_lidos"
    ] == (
    ...
    with extracted_comt(archive) as comt:
        ...
        try:
            result = transform_rows(
                ...
                year=year,
                allowed_ufs=allowed_ufs,
                municipality_map=municipality_map,
                south_municipalities=south_municipalities,
